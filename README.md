# sql-practice
SQL practice solutions from HackerRank |Notes| Targeting Data Analyst roles | Updated daily
# SQL Practice — HackerRank

![SQL](https://img.shields.io/badge/SQL-Practice-blue)
![HackerRank](https://img.shields.io/badge/Platform-HackerRank-green)
![Status](https://img.shields.io/badge/Status-Active-brightgreen)

## About
SQL practice solutions from HackerRank.
Building a strong foundation in SQL for a Data Analyst career.

## Topics Covered
SELECT · WHERE · GROUP BY · ORDER BY · JOINS · Subqueries

## Tools Used
- SQL (MySQL)
- HackerRank

## Connect With Me
- LinkedIn: [https://www.linkedin.com/in/jagruthi-sachin-poondrrikaksha]
- GitHub: [https://github.com/jagruthi315]
